...
function createFieldStorage() {
	...
	if( fields.length && ! window.localStorage.getItem('fieldData') ) {
		var fieldData = [];
...
function renderSectionHandles() {
  ...
  document.querySelectorAll('.fld-tab').forEach(function(tab) {
      tab.style.cssText = "height: auto; width: auto;";
  });
}
...
function setHandleButtonLabel() {
	if( window.localStorage.getItem('cpShowHandles') ) {
		document.body.classList.add('showHandles');
		buttonLabel = "Hide handles";
	} else {
		buttonLabel = "Show handles";
	}
}

setHandleButtonLabel();

function renderHandleButton() {
	
	var buttonVar = '<div class="handleButton btn submit">'+buttonLabel+'</div>';
	document.querySelector('#crumbs').insertAdjacentHTML('beforeEnd', buttonVar);
